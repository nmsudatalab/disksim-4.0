...
static int DISKSIM_LOGORG_PRINT_MAPREQUESTS_depend(char *bv) {
return -1;
}

static void DISKSIM_LOGORG_PRINT_MAPREQUESTS_loader(struct logorg * result, int i) { 
if (! (RANGE(i,0,1))) { // foo 
 } 
 result->printmaprequests = i;
...
(void *)DISKSIM_LOGORG_PRINT_MAPREQUESTS_loader
};
...
DISKSIM_LOGORG_PRINT_MAPREQUESTS_depend
};
